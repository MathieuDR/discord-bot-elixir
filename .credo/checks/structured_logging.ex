defmodule Extensions.Credo.StructuredLogging do
  ...
  use Credo.Check,
    ...
    explanations: [
      check: "Collects Logger calls that interpolate values into their message instead of using metadata.",
      params: []
    ]
  ...
  defp check_ast({{:., meta, [module, function]}, _, arguments} = ast) do
    if logger?(module) and unstructured_log?(function, arguments) do
      {:error,
       message: ~s'[Structured Logging] Use metadata instead of interpolating: #{abbreviate_call(ast)}',
